...
def draw_scanpath(fix_x, fix_y, fix_d, alpha=1, invert_y=False, ydim=None):
    ...
    for i in range(len(fix_x)):
        if i == 0:
            plt.plot(fix_x[i], fix_y[i], marker='o', ms=23, mfc=COLORS['skyblue'][0], mec='black', alpha=0.7)
        elif i == len(fix_x) - 1:
            plt.plot(fix_x[i], fix_y[i], marker='o', ms=23, mfc=COLORS['scarletred'][0], mec='black', alpha=0.7)
        else:
            plt.plot(fix_x[i], fix_y[i], marker='o', ms=23, mfc=COLORS['aluminium'][0], mec='black', alpha=0.7)
